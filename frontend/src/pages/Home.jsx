...
import { Search, MapPin, Monitor, Shirt, Book, Home as HomeIcon, Palette, Package, Check, SlidersHorizontal, Recycle, Handshake, RefreshCw, Star, X } from 'lucide-react';
...
const Home = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                ...
                showToast('Eşyaları şu an getiremiyoruz, lütfen birazdan tekrar deneyin.', 'error');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleJoinClick = async (e, id) => {
        ...
        try {
            ...
            setItems(items.map(i => i.id === id ? { ...i, applicationsCount: (i.applicationsCount || 0) + 1 } : i));
        } catch (err) {
            const msg = err.response?.data?.message || 'Bir hata oluştu.';
            if (msg.includes('already applied') || msg.includes('has already applied')) {
    ...
    const removeFilter = (type, value) => {
        switch(type) {
            case 'category': setActiveCategories(prev => prev.filter(v => v !== value)); break;
            case 'city': setActiveCities(prev => prev.filter(v => v !== value)); break;
            case 'district': setActiveDistricts(prev => prev.filter(v => v !== value)); break;
        }
    };
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <section className="bg-emerald-900 text-white py-20 relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
                    ...
                    >
                        Paylaş, Yeni Bir Hikaye Başlasın.
                        Tamamen şeffaf ve güvenilir bir topluluk döngüsü.
                    </motion.p>
                    ...
                    >
                        ...
                        >
                            ...
                            {(activeCategories.length + activeCities.length + activeDistricts.length) > 0 && (
                    ...
                    {(activeCategories.length > 0 || activeCities.length > 0 || activeDistricts.length > 0) && (
                        <div className="max-w-3xl mx-auto mt-4 flex flex-wrap gap-2 items-center justify-center">
                            {activeCategories.map(cat => (
                                <span key={cat} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 backdrop-blur-sm border border-emerald-300/30 text-emerald-50 text-sm font-medium shadow-sm">
                                    {cat}
                                    <button onClick={() => removeFilter('category', cat)} className="hover:text-white transition-colors"><X className="w-3.5 h-3.5" /></button>
                                </span>
                            ))}
                            {activeCities.map(city => (
                                <span key={city} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 backdrop-blur-sm border border-emerald-300/30 text-emerald-50 text-sm font-medium shadow-sm">
                                    {city}
                                    <button onClick={() => removeFilter('city', city)} className="hover:text-white transition-colors"><X className="w-3.5 h-3.5" /></button>
                                </span>
                            ))}
                            {activeDistricts.map(dist => (
                                <span key={dist} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 backdrop-blur-sm border border-emerald-300/30 text-emerald-50 text-sm font-medium shadow-sm">
                                    {dist}
                                    <button onClick={() => removeFilter('district', dist)} className="hover:text-white transition-colors"><X className="w-3.5 h-3.5" /></button>
                                </span>
                            ))}
                            <button 
            ...
            <main className="w-full px-6 lg:px-12 xl:px-24 2xl:px-32 py-8 sm:py-12">
                <div className="flex flex-col items-center justify-center gap-4 mb-10 text-center">
                    <div className="space-y-1.5">
                        ...
                        >
                            <h2 className="text-2xl md:text-3xl font-black text-slate-800 tracking-tight flex items-center gap-2">
                                {activeTab === 'donation' ? (
                                    <>Döngüdeki Eşyalar <span className="text-emerald-500 animate-pulse text-xl">✨</span></>
                                ) : activeTab === 'exchange' ? (
                                    <>Takas Havuzu <RefreshCw className="text-emerald-600 w-5 h-5 animate-spin-slow" /></>
                                ) : (
                                    <>Tüm Paylaşımlar <span className="text-emerald-500 text-xl">🌍</span></>
                                )}
                            </h2>
                    ...
                    {/* Quick Mode Selector (Tabs) - More Compact with All Option */}
                    <div className="relative p-1 bg-slate-100/50 backdrop-blur-md rounded-full border border-slate-200/60 shadow-inner flex items-center gap-1 w-full max-w-[360px] mx-auto">
                        {[
                            ...
                            >
                                ...
                                <span className="relative z-10 flex items-center gap-1.5">
                                    <tab.icon className={`w-3.5 h-3.5 ${activeTab === tab.id ? 'opacity-100' : 'opacity-60'}`} />
                                    {tab.label}
                                </span>
                            </motion.button>
                        ))}
                    ...
                    <>
                        {filteredItems.length === 0 ? (
                            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
                                <p className="text-slate-400 text-lg">Aradığınız kriterlere uygun eşya bulunamadı.</p>
                                <button
                                    onClick={clearAllFilters}
